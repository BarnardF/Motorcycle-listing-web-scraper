"""
ZABikers Cached Tracker
Searches locally cached ZABikers listings (from cache_zabikers.py).
...
def load_cache():
    """Load ZABikers cache from file"""
    try:
        if not Path(ZABIKERS_CACHE_FILE).exists():
            logger.warning(f"[{SOURCE}] Cache file not found: {ZABIKERS_CACHE_FILE}")
            logger.warning(f"[{SOURCE}] Run 'python cache_zabikers.py' to create cache first")
            return {}
...
def is_relevant_listing(title, search_term):
    """
    Check if listing is relevant to search term using fuzzy matching.
    
    Args:
        title: Full listing title
        search_term: Bike model to search for
        
    Returns:
        bool: True if listing matches search term
    """
    threshold = MATCH_THRESHOLDS.get("zabikers", 0.45)
    
    # Try matching against full title
...
def scrape_zabikers_cached(search_term):
    """
    Search ZABikers cache for listings matching search_term.
    ...
    for variation in search_variations:
        for listing_id, listing in cached_listings.items():
            ...
            try:
                ...
                if is_relevant_listing(title, variation):
...
def scrape_zabikers(search_term):
    """Wrapper for main.py compatibility"""
    return scrape_zabikers_cached(search_term)